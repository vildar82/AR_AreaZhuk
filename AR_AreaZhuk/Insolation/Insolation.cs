...
namespace AR_AreaZhuk
{
    public class Insolation
    {
        ...
        public Insolation ()
        {
            ...
            RoomInsulations.Add(new RoomInsulation("Двухкомнатная", 2, new List<string>() { "1=C", "1=D", "2=C", "2=B|C", "2=B" }));
            RoomInsulations.Add(new RoomInsulation("Трехкомнатная", 3, new List<string>() { "2=C", "2=D", "1=C", "1=D", "2=B" }));
            RoomInsulations.Add(new RoomInsulation("Четырехкомнатная", 4, new List<string>() { "2=C", "2=D" }));
        }
                        ...
                        else if (indexRow != 0 & topFlat.SelectedIndexBottom > 0)               //первая слева квартира
                        {
                            ...
                            foreach (var ln in lightNiz)
                            {
                                ...
                                if (isCorner)
                                {
                                    ...
                                    ins = insulation.Matrix[indexColumn + indexColumnLLUBottom, indexRow - Math.Abs(ln) * direction];
                                else ins = insulation.Matrix[indexColumn - Math.Abs(ln) * direction, indexRow + indexColumnLLUBottom];
                                if (string.IsNullOrWhiteSpace(ins)) continue;
